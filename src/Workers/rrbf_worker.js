const rrbf_worker = () => {
    ...
    const SEND_UPDATE_ON_NTH_TIME = 100; // Can be adjusted based on UI performance requirements
    let interval = -1
    let frame = []
    let counter = 0;
                ...
                if(interval < 0){ 
                    ...
                    interval = setInterval(() => { 
                            if(frame.length < MAX_FRAME_SIZE){
                                // intially when frame size is less, we send every update, i.e., every 1ms
                                ...
                                frame.push(createRabdomRow());
                                frame.shift();
                                // every SEND_UPDATE_ON_NTH_TIME * 100 milliseconds(that is every SEND_UPDATE_ON_NTH_TIME mesage, we update the UI), this is to adjust on performance
